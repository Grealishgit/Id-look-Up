...
const KraApplications = () => {
    ...
    return (
        <div className={`mt-20 ${isDarkMode ? "text-white" : "text-black"} px-4`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-4">
                {stats.map((stat, index) => (
                    <div key={index} className={`p-4 rounded-lg ${isDarkMode ? "bg-gray-500" : "bg-white"} shadow flex flex-col`}>
                        <div className={`w-10 h-10 flex items-center justify-center rounded-full  ${stat.iconBg}`}>
                            {stat.icon}
                        </div>
                        <h4 className=" text-sm mt-2">{stat.title}</h4>
                        <h2 className="text-2xl font-semibold mt-1">{stat.value}</h2>
                        <svg className="w-full h-8 mt-2" viewBox="0 0 100 20">
                            <path d="M0,10 Q40,5 40,15 T80,10" className={`fill-none ${stat.graphColor}`} strokeWidth="2" />
                        </svg>
